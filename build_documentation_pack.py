...
def add_title_page(doc):
    ...
    sub_run = subtitle.add_run(
        "Combined Technical Design Document, Architecture Diagram, Flow Diagram, Test Plan, and Test Results"
    )
    ...
    add_body(
        doc,
        "Care Nova AI is an offline-first healthcare intelligence asset that accepts patient questions, vitals, profile data, and prior context, then routes the request through governed specialist agents to produce safety-first guidance, care summaries, and healthcare or payer workflow support. The current package combines the core design, runtime flow, and current validation evidence into a single handoff artifact.",
        after=8,
    )
