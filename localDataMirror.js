...
export async function getLocalDataMirrorStatus(env = process.env) {
  ...
  const manifestPath = resolveMirrorManifestPath(env);
  let manifest = null;
...
export async function syncLocalDataMirror(reason = "manual-sync", env = process.env, options = {}) {
  ...
  await writeFile(resolve(mirrorDir, mirrorManifestName), `${JSON.stringify(manifest, null, 2)}\n`, "utf8");
...
async function readMirrorManifest(mirrorDir) {
  try {
    const manifest = JSON.parse(await readFile(resolve(mirrorDir, mirrorManifestName), "utf8"));
